...
import static org.vash.vate.net.openhft.hashing.Util.*;
...
public abstract class LongTupleHashFunction implements Serializable {
    ...
    private static final Access<byte[]> ARRAY_ACCESS = UnsafeAccess.INSTANCE;
    ...
    private static void unsafeHash(final LongTupleHashFunction f, final byte[] input,
                                   final long off, final long len, final long[] result) {
        f.hash(input, ARRAY_ACCESS, off, len, result);
    }
}

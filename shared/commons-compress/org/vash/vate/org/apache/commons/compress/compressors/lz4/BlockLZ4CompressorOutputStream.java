 ...
 * under the License.
 */
package org.vash.vate.org.apache.commons.compress.compressors.lz4;
...
import org.vash.vate.org.apache.commons.compress.compressors.CompressorOutputStream;
import org.vash.vate.org.apache.commons.compress.compressors.lz77support.LZ77Compressor;
import org.vash.vate.org.apache.commons.compress.compressors.lz77support.Parameters;
import org.vash.vate.org.apache.commons.compress.compressors.lz77support.LZ77Compressor.Block;
import org.vash.vate.org.apache.commons.compress.compressors.lz77support.LZ77Compressor.Callback;
import org.vash.vate.org.apache.commons.compress.utils.ByteUtils;
...
public class BlockLZ4CompressorOutputStream extends CompressorOutputStream {
    ...
    final static class Pair {
        ...
        byte[] addLiteral(final LZ77Compressor.LiteralBlock block) {
            final byte[] copy = VTArrays.copyOfRange(block.getData(), block.getOffset(),
        ...
        private void prependTo(final Pair other) {
            final Iterator<byte[]> listBackwards = createDescendingIterator(literals);
    ...
    */
    ...
    private final LinkedList<byte[]> expandedBlocks = new LinkedList<byte[]>();
    ...
    private void clearUnusedPairs() {
        int pairLengths = 0;
        int pairsToKeep = 0;
        for (final Iterator<Pair> it = createDescendingIterator(pairs); it.hasNext(); ) {
    ...
    public void prefill(final byte[] data, final int off, final int len) {
        if (len > 0) {
            final byte[] b = VTArrays.copyOfRange(data, off, off + len);
    ...
    private Pair writeBlocksAndReturnUnfinishedPair(final int length) throws IOException {
        ...
        Pair last = pairs.isEmpty() ? null : pairs.getLast();
    ...
    private static final <T> Iterator<T> createDescendingIterator(List<T> list)
    {

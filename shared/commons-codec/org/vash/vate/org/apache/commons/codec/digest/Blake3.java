 ...
 *      byte[] txKey = VTArrays.copyOfRange(keys, 0, 32);
 *      byte[] rxKey = VTArrays.copyOfRange(keys, 32, 64);
 * }</pre>
 * <p>
...
public final class Blake3 {
    ...
    private static final class Output {
        ...
        private void rootOutputBytes(final byte[] out, int offset, int length) {
            ...
            while (length > 0) {
                ...
                int wordCounter = 0;
                while (chunkLength > 0) {
                    final int wordLength = Math.min((Integer.SIZE / Byte.SIZE), chunkLength);
    ...
    private static final int BLOCK_INTS = BLOCK_LEN / (Integer.SIZE / Byte.SIZE);
    private static final int KEY_LEN = 32;
    private static final int KEY_INTS = KEY_LEN / (Integer.SIZE / Byte.SIZE);
    ...
    private static void checkBufferArgs(final byte[] buffer, final int offset, final int length) {
        VTObjects.requireNonNull(buffer, "buffer");
        if (offset < 0) {
    ...
    public static Blake3 initKeyDerivationFunction(final byte[] kdfContext) {
        VTObjects.requireNonNull(kdfContext, "kdfContext");
    ...
    public static Blake3 initKeyedHash(final byte[] key) {
        VTObjects.requireNonNull(key, "key");
    ...
    private static int[] unpackInts(final byte[] buf, final int nrInts) {
        ...
        for (int i = 0, off = 0; i < nrInts; i++, off += (Integer.SIZE / Byte.SIZE)) {

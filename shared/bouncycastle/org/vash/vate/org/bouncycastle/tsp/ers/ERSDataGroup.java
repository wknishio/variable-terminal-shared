    ...
    extends ERSCachingData
{
    ...
    {
        this.dataObjects = new ArrayList (dataObjects.length);
     ...
     */
    public ERSDataGroup(List  dataObjects)
    {
        this.dataObjects = new ArrayList (dataObjects.size());
    ...
    {
        List  hashes = getHashes(digestCalculator, previousChainHash);
        if (hashes.size() > 1)
        {
    ...
    protected byte[] calculateHash(DigestCalculator digestCalculator, byte[] previousChainHash)
    {
        ...
        if (hashes.size() > 1)
        {
            List  dHashes = new ArrayList (hashes.size());

...
import org.vash.vate.org.bouncycastle.util.Iterable;
        ...
        RecipientId selector)
    {
        if (selector instanceof PKIXRecipientId)
        {
            ...
            if (issuer != null && subjectKeyId != null)
            {
                ...
                List  match1 = (ArrayList )table.get(new PKIXRecipientId(pkixId.getType(), issuer, pkixId.getSerialNumber(), null));
                if (match1 != null)
                {
                    results.addAll(match1);
                }

                Collection  match2 = (ArrayList )table.get(new PKIXRecipientId(pkixId.getType(), null, null, subjectKeyId));
                if (match2 != null)
                {
